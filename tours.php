...
$search_results = $tourcms->search_tours($params, $channel_id);

$formatted_results = [];

if ($search_results) {
	foreach ((object)$search_results as $result) {
		if (!empty($result->tour_name)) {
			$formatted_results[] = [
				'tour_name' => (string)$result->tour_name,
				'tour_url' => (string)$result->tour_url,
				'summary' => (string)$result->summary,
				'thumbnail_image' => (string)$result->thumbnail_image,
				'duration' => (string)$result->duration_desc,
				'location' => (string)$result->location,
				'from_price' => (string)$result->from_price,
			];
		}
	}
}
				...
				<div class="row">
					<div id="container">
						...
						<div class="row">
							...
							<?php foreach ($formatted_results as $tour): ?>
										...
										<div class="tourcms-card-horizontal">
												...
												<div id="c-49-d" class="carousel slide" data-interval="false">
													<div class="carousel-inner">
														<div class="carousel-item active">
															<img class="d-block w-100" src="<?= $tour['thumbnail_image'] ?>" alt="">
														</div>
													</div>
												</div>
											</div>
											...
											<div class="card-body">
												...
												<div class="card-meta-right">
													...
													<p class="tourcms-card-text price-label" style=""><?= $tour['location'] ?></p>
													<p class="tourcms-card-text tourcms_wp_summary"><?= $tour['summary'] ?></p>
													<p class="tourcms-card-text"> <b>Duration:</b><?= $tour['duration'] ?></p>
													<div class="right price-panel">
														<p class="tourcms-card-text price"><span class='fromprice'><a href="<?= $tour['tour_url'] ?>" rel="tag">Book now from &#8364;<?= $tour['from_price'] ?></a></span></p>
													</div>
								...
								<div class="col-12 col-md-6 col-lg-4 col-xl-3">
									<div class="card tour-mobile">
										...
										<div class="card-footer bg-transparent">
											<p class="tourcms-card-text tour-mobile-footer-left"> <b>Duration:</b> <?= $tour['duration'] ?><br></p>
											<p class="tourcms-card-text tour-mobile-footer-right" style=""><a href="https://wordpress.tourcms.com/tours/day-tour-example-1-departure-a-day-inline-booking/">Book now <span class='fromprice'>from &#8364;<?= $tour['from_price'] ?></span></a><br></p>
										</div>
									</div>
								</div>
							<?php endforeach; ?>
...
<style type="text/css">
	...
	/* En pantallas pequeñas, oculta la versión para escritorio y muestra la de móvil */
	@media (max-width: 768px) {
		.tour-desktop {
			display: none;
		}
